...
function findBook(books,wanted){
 const w=compact(wanted);const aliases=BOOK_ALIASES[w]||[wanted];
 for(const a of aliases){const ac=compact(a);const exact=books.find(b=>compact(b.name)===ac);if(exact)return exact;}
 for(const a of aliases){const an=norm(a);const partial=books.find(b=>{const bn=norm(b.name);return bn&&an&&(bn.includes(an)||an.includes(bn));});if(partial)return partial;}
 return null;
}
...
module.exports=async function handler(req,res){
 const key=process.env.API_FOOTBALL_KEY||process.env.API_SPORTS_KEY;const exchangeKey=process.env.ODDSMATCHER_ODDS_API_KEY;if(!key)return res.status(500).json({ok:false,error:'API_FOOTBALL_KEY non configurata'});
 ...
 try{
  const h={'x-apisports-key':key};const br=await fetch('https://v3.football.api-sports.io/odds/bookmakers',{headers:h});const bj=await br.json();const books=Array.isArray(bj.response)?bj.response:[];const target=findBook(books,bookmaker);
  if(bookmaker&&!target)return res.status(400).json({ok:false,error:'Bookmaker non disponibile in API-Football',requested:bookmaker,available:books.map(b=>b.name).filter(Boolean)});
  ...
  const fixtureMap=new Map();try{const fr=await fetch('https://v3.football.api-sports.io/fixtures?date='+encodeURIComponent(date)+'&timezone=Europe/Rome',{headers:h});const fj=await fr.json();if(!(fj.errors&&Object.keys(fj.errors).length))for(const f of(fj.response||[]))if(f?.fixture?.id)fixtureMap.set(String(f.fixture.id),f);}catch{}
  ...
  out.sort((a,b)=>(b.rating||0)-(a.rating||0)||b.back-a.back);const resultIds=[...new Set(out.map(x=>String(x.fixtureId)))];const resolved=resultIds.filter(id=>fixtureMap.has(id));res.setHeader('Cache-Control','s-maxage=30, stale-while-revalidate=10');
  return res.status(200).json({ok:true,date,bookmaker:target?.name||bookmaker||'Tutti',requestedBookmaker:bookmaker,count:out.length,exchangeConnected:!!exchangeKey,exchangeEvents:exchangeEvents.length,exchangeMatched:matched,exchangeError,resolvedFixtures:resolved.length,totalFixtures:resultIds.length,note:exchangeKey?(exchangeError?'Errore Exchange: '+exchangeError:'Betfair Exchange collegato tramite The Odds API. Abbinamenti trovati: '+matched+'.'):'ODDSMATCHER_ODDS_API_KEY non configurata.',results:out});
