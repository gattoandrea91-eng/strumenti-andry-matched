...
async function fotmobFetch(url) {
  ...
  if (!response.ok) {
    throw new Error(`FotMob ${response.status}: ${text.slice(0, 300)}`);
  }
...
function getLiveMinute(status) {
  ...
  const candidates = [
    ...stringCandidates(status.liveTime),
    ...stringCandidates(status.reason),
    ...stringCandidates(status.matchTime),
    ...stringCandidates(status.time)
  ];
  ...
  for (const raw of candidates) {
    ...
    const minuteMatch = s.match(/(\d{1,3})(?:\s*['’]|\s*min|\s*$)/i);
    if (minuteMatch) return `${Number(minuteMatch[1])}'`;

    const addedMatch = s.match(/(\d{1,2})\s*\+\s*(\d{1,2})/);
    if (addedMatch) return `${Number(addedMatch[1]) + Number(addedMatch[2])}'`;
  }
...
function statusReasonText(status) {
  const vals = stringCandidates(status?.reason);
  return vals.join(" ") || "";
}
...
module.exports = async (req, res) => {
  try {
    ...
    if (matchId) {
      ...
      const events = content?.matchFacts?.events || content?.matchFacts?.events?.events || [];
      ...
      return res.status(200).json({
        ...
        minute: data?.general?.matchTime || data?.general?.liveTime || "",
        stats,
        rtg: calculateRtg(stats),
        momentum: {
          latest: lastMomentumValue(momentum),
          history: normalizeMomentum(momentum)
        },
        events,
        shotmap,
    ...
    const data = await fotmobFetch(
      `${FOTMOB_BASE}/matches?date=${encodeURIComponent(date)}&includeNextDayLateNight=true`
    );
    ...
    for (const league of leagues) {
      ...
      for (const match of leagueMatches) {
        ...
        matches.push({
          ...
          minute,
          reason: statusReasonText(status),
