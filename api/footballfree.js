...
function extractPairFromNode(node){if(!node||typeof node!=="object")return null;const candidates=[node.stats,node.values,node.value,node.data];for(const c of candidates){if(Array.isArray(c)&&c.length>=2){const unwrap=v=>v&&typeof v==="object"?(v.value??v.stat??v.displayValue??0):v;return [num(unwrap(c[0])),num(unwrap(c[1]))]}}if(node.home!==undefined||node.away!==undefined)return [num(node.home?.value??node.home),num(node.away?.value??node.away)];if(node.homeValue!==undefined||node.awayValue!==undefined)return [num(node.homeValue),num(node.awayValue)];return null}
function findStatFlexible(root,aliases){const wanted=aliases.map(normalizeKey).filter(Boolean),queue=[root],visited=new Set();while(queue.length){const node=queue.shift();if(!node||typeof node!=="object"||visited.has(node))continue;visited.add(node);if(!Array.isArray(node)){const names=[node.key,node.title,node.name,node.label,node.statName,node.localizedTitle].map(normalizeKey).filter(Boolean);const matches=names.some(name=>wanted.some(w=>name===w||name.includes(w)||w.includes(name)));if(matches){const pair=extractPairFromNode(node);if(pair)return pair}for(const [k,v] of Object.entries(node)){const nk=normalizeKey(k);if(wanted.some(w=>nk===w||nk.includes(w)||w.includes(nk))){if(Array.isArray(v)&&v.length>=2)return [num(v[0]?.value??v[0]),num(v[1]?.value??v[1])];if(v&&typeof v==="object"){const p=extractPairFromNode(v);if(p)return p}}}}if(Array.isArray(node))for(const item of node)queue.push(item);else for(const value of Object.values(node))if(value&&typeof value==="object")queue.push(value)}return [0,0]}
...
function shotmapStats(shotmap){const shots=Array.isArray(shotmap?.shots)?shotmap.shots:Array.isArray(shotmap)?shotmap:[];let hShots=0,aShots=0,hSot=0,aSot=0;for(const s of shots){const isHome=s.isHome===true||s.teamId===shotmap?.homeTeamId;if(isHome)hShots++;else aShots++;const event=normalizeKey(s.eventType||s.result||s.type||s.shotType);const onTarget=event.includes("goal")||event.includes("save")||event.includes("attemptsaved")||event.includes("ontarget");if(onTarget){if(isHome)hSot++;else aSot++}}return {totalShots:{home:hShots,away:aShots,total:hShots+aShots},shotsOnTarget:{home:hSot,away:aSot,total:hSot+aSot}}}
...
module.exports=async(req,res)=>{try{
 ...
 if(matchId){
  ...
  const stats={possession:pairFlexible(statsRoot,["BallPossesion","BallPossession","Possession","Possesso","Possesso palla"]),totalShots:pairFlexible(statsRoot,["total_shots","TotalShots","Total shots","Shots total","Tiri","Tiri totali","Total attempts"]),shotsOnTarget:pairFlexible(statsRoot,["ShotsOnTarget","shots_on_target","Shots on target","On target","Tiri in porta","Tiri nello specchio"]),shotsOffTarget:pairFlexible(statsRoot,["ShotsOffTarget","shots_off_target","Shots off target","Tiri fuori"]),corners:pairFlexible(statsRoot,["corners","Corner","Corner kicks","Calci d'angolo"]),touchesBox:pairFlexible(statsRoot,["touches_opp_box","TouchesInOppositionBox","Touches in opposition box","Touches in box","Tocchi in area"]),bigChances:pairFlexible(statsRoot,["big_chance","BigChances","Big chances","Grandi occasioni"]),bigChancesMissed:pairFlexible(statsRoot,["big_chance_missed_title","BigChancesMissed","Big chances missed","Grandi occasioni sbagliate"])};
  const sm=shotmapStats(shotmap);if(stats.totalShots.total===0&&sm.totalShots.total>0)stats.totalShots=sm.totalShots;if(stats.shotsOnTarget.total===0&&sm.shotsOnTarget.total>0)stats.shotsOnTarget=sm.shotsOnTarget;
