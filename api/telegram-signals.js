...
module.exports=async(req,res)=>{
 try{
  if(req.query?.test==='1'){ await telegramSend('✅ <b>SoccerTrend Telegram attivo</b>\n\nRating adattivo attivo: WATCH 55+, segnale 66+, forte 78+.'); return res.status(200).json({success:true,mode:'TEST',sent:true}); }
  ...
  return res.status(200).json({success:true,mode:'ADAPTIVE_OVER_V2',telegramThreshold:66,strongThreshold:78,watchThreshold:55,live:matches.length,candidates:candidates.length,alerts,checked});
